 ...
 * 14. presets.fetchHostRegex() → { saved: [{ kind: 'global'|'preset'|'scoped', name, count, updated }] }
 *       读取酒馆正则扩展的全部规则，按「全局正则」「预设正则 · 酒馆预设名」「局部正则 · 角色卡名」存成导出预设，同名覆盖；没有正则的类别不存
 *
    ...
    await run(event.currentTarget, async () => {
      try {
        ...
        YaKitToast.show(saved.length
          ? `已获取：${saved.map((item) => `${item.name.split(' · ')[0]} ${item.count} 条`).join('、')}`
          : '酒馆里没有读到正则', saved.length ? 'success' : 'warning');
      } catch (error) {
