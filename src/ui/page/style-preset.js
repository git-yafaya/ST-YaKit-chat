...
(() => {
  ...
  // 文风正文的结构模板：接在已有正文末尾，不覆盖用户写过的内容
  const TEMPLATE = [
    '## 文风目标与适用范围',
    '（想要什么样的读感；只在原文有对应内容时生效）',
    '',
    '## 视角与叙述距离',
    '（例如：保持原文视角，不改成全知旁白）',
    '',
    '## 句式、段落与节奏',
    '（例如：段落偏短，长句拆开，情绪转折单独成行）',
    '',
    '## 对白口吻与心理描写',
    '（例如：对白保留人物口吻，心理描写不用「他心想」领起）',
    '',
    '## 表达取舍',
    '（想多用、少用或不用的说法；换说法时不删掉原文的信息）',
    '',
    '## 对照范例',
    '范例只用来校准写法，不写进结果，也不增加原文没有的动作和事实。',
    '平版：她把杯子放在桌上，说自己不累。',
    '本文风：她把杯子放到桌上。“我不累。”',
    '学什么：同一件事拆成动作和对白，不添新动作。',
  ].join('\n');
