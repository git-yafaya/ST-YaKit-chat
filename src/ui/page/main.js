...
(() => {
  ...
  function applyTheme({ theme, tavern }) {
    ...
    root.style.removeProperty('--yakit-font');
    ...
    root.style.setProperty('--yakit-font', tavern.font);
  ...
  function receive(data = {}) {
    if (data.type === 'yakit:theme') {
      applyTheme(data);
      window.dispatchEvent(new CustomEvent('yakit-theme', { detail: data }));
    }
    ...
    if (data.type === 'yakit:tab') {
      showTab(data.tab);
      window.dispatchEvent(new CustomEvent('yakit-tab', { detail: data }));
    }
  }

  // 弹窗优先直接调用 yakitReceive（同一帧更新）；也兼容 postMessage
  window.yakitReceive = receive;
