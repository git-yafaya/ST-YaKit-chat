...
const fields = ['profile', 'jailbreak', 'prompt'];
...
function getGroup(settings, kind, field) {
    if (field === 'profile') return settings.apiProfiles;
    return settings.prompts[field === 'jailbreak' ? 'jailbreak' : kind === 'regex' ? 'regex' : 'style'];
}

function validChoice(settings, kind, field, value) {
    return typeof value === 'string' && (value === 'follow'
        || value === (field === 'profile' ? 'main' : 'none')
        || getGroup(settings, kind, field).items.some(item => item.id === value));
}
...
function readAssistant(settings, kind) {
    ...
    return Object.fromEntries(fields.map(field => [field,
        validChoice(settings, kind, field, saved?.[field]) ? saved[field] : 'follow',
    ]));
}
...
export function getAssistantSelection(kind) {
    ...
    return {
        ...
        jailbreak: resolvePrompt(settings, kind, 'jailbreak', selection.jailbreak),
        prompt: resolvePrompt(settings, kind, 'prompt', selection.prompt),
    };
}

// 删除事务只清理现有助手的匹配项，不补建助手配置。
export function resetAssistantReferences(settings, field, id, kind) {
    if (!fields.includes(field)) throw new Error('这项助手设置不能修改');
    if (field === 'prompt') assertKind(kind);
