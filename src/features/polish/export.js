...
export async function exportPolish(job, options = {}) {
    ...
    const messages = [...job.segments].sort((a, b) => a.index - b.index).map(segment => ({ mes: segment.polished }));
    const now = new Date();
    ...
    if (format === 'txt') {
        ...
        const blob = await buildEpub(chapters, { labelMode: 'plain', characterName, now, identifier: utilities.uuidv4(), JSZip: globalThis.JSZip });
